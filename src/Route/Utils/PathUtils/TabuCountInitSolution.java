...
public class TabuCountInitSolution {
    private MyGraph graph;
    private List<Flow> flowList;
    private int hyperPeriod = 1;

    public TabuCountInitSolution(MyGraph graph, List<Flow> flowList) {
        this.graph = graph;
...
//        int[][][][][] initSolution = new int[flowList.size()][6][graph.point.length][graph.point.length][hyperPeriod];
        ...
        for (int i = 0; i < flowList.size(); i++) {
            ...
            for (int j = 0; j < flowList.get(i).countPath.redundantPath.size(); j++) {
...
//            for (int j = 0; j < flowList.get(i).redundantPath.size(); j++) {
            List<List<LinkUse>> tempSolution = NavigationUtil.deepCloneSolution(initSolution.get(i));
            int[][][] tempLinkSlotUse = NavigationUtil.deepCloneArr(linkSlotUse);
            int groupFlag = 0;
            //第j条冗余路径的第k条路径
            for (int k = 0; k < flowList.get(i).countPath.redundantPath.size(); k++) {
                //将路径转化为链路格式
                List<Link> linkPathList = Link.getLinks(flowList.get(i).countPath.redundantPath.get(k));
                eachStart:
                //遍历每一个开始时间
